...
class FastMapping : public AbstractFTL {
 private:
  ...
  std::unordered_map<uint64_t, uint32_t> bmap;  //block-level mapping
  struct LogMap {
    uint64_t freePageCnt;
    uint64_t pbn;
    std::vector<uint32_t> lpn;
    std::vector<uint32_t> lpnStat;  //-1 for invalid, 0 for free, 1 for valid
    uint64_t dbn;  //data block number, for SW only
  };

  uint64_t SWflag; //  SW是否已经分配
  uint64_t currRW; //  currently writing RW block num
  std::vector<LogMap> logbmap;
  
  uint64_t getRW();
  void switchMerge(uint64_t, uint64_t, uint64_t &);
  void partialMerge(uint64_t, uint64_t, uint64_t, uint64_t, uint64_t &);
  void fullMergeSW(uint64_t, uint64_t &);
  void fullMergeRW(uint64_t, uint64_t &);
